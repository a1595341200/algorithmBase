...
public:
    ...
    template <class Func,
              typename std::enable_if<
                  std::is_convertible<decltype(std::declval<Func>()(std::declval<T>()...)),
    ...
    template <class Func,
              typename std::enable_if<
                  std::is_void<decltype(std::declval<Func>()(std::declval<T>()...))>::value,
                  int>::type = 0>
    void connect(Func callback) {
        m_callbacks.push_back([callback = std::move(callback)](T... t) mutable {  // define t
            callback(std::forward<T>(t)...);                                      // use t
...
#define PENGSIG_FUN(_fun)                                \
    [=](auto &&... _t) {                                 \
        return __fun(std::forward<decltype(_t)>(_t)...); \
    }
#endif
